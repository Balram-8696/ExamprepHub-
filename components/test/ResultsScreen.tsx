...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ result, totalQuestions, onViewSolutions, onBackToTests }) => {
    ...
    const chartData = [...lastAttempts].reverse().map((attempt, index) => ({
        date: attempt.submittedAt ? new Date((attempt.submittedAt as Timestamp).seconds * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : `Attempt ${index + 1}`,
