...
const AttemptHistoryItem: React.FC<{
    ...
    const formatDate = (timestamp: Timestamp | Date) => {
        ...
        const date = (timestamp as Timestamp).seconds ? new Date((timestamp as Timestamp).seconds * 1000) : (timestamp as Date);
...
const HistoryPage: React.FC<HistoryPageProps> = ({ onInitiateTestView }) => {
    ...
    useEffect(() => {
        if (user) {
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
                ...
                // FIX: Changed from `Object.values` to `Object.keys` to avoid a TypeScript issue where
                // the iterated item `group` was incorrectly typed as `unknown`.
    ...
    const sortedTestIds = Object.keys(groupedResults).sort((a, b) => {
        ...
        if (!lastAttemptA || !lastAttemptB) return 0;
        return (lastAttemptB as Timestamp).toDate().getTime() - (lastAttemptA as Timestamp).toDate().getTime();
    });
