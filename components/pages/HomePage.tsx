...
import { Shield, ArrowRight, Star, Megaphone, X, Quote, ChevronDown, TrendingUp, HelpCircle, Info } from 'lucide-react';
import SkeletonCard from '../skeletons/SkeletonCard';
import DynamicIcon from '../layout/DynamicIcon';
import SkeletonHomePage from '../skeletons/SkeletonHomePage';

interface HomePageProps {
    onInitiateTestView: (details: { test: Test; action: 'start' | 'resume' | 'result' | 'practice'; resultData?: UserResult }) => void;
    categories: Category[];
            ...
            <div className="max-w-3xl mx-auto space-y-4">
                {config.faqs.map((f, i) => (
                    <div key={i} className="border dark:border-gray-700 rounded-lg">
                        <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex justify-between items-center p-4 text-left font-semibold text-gray-800 dark:text-gray-200">
...
const HomePage: React.FC<HomePageProps> = ({ 
    ...
    useEffect(() => {
        if (!user || userResults.length === 0 || allTests.length === 0) {
            setRecommendedTests([]);
        ...
        if (weakestCategory) {
            ...
            const recommendations = allTests
                .filter(test => 
                    test.categoryId === weakestCategory!.id && 
                    !takenTestIds.has(test.id) &&
                    isTestLive(test)
                )
                .slice(0, 4);
    ...
    const handleTestAction = async (testId: string, action: 'start' | 'resume' | 'result') => {
        ...
        if (action === 'start') {
            ...
            onInitiateTestView({ test, action: 'resume' });
        } else if (action === 'result') {
            const resultData = userResults.find(r => r.testId === testId);
            const testDoc = await getDoc(doc(db, 'tests', testId));
            if(testDoc.exists()) {
                onInitiateTestView({ test: {id: testDoc.id, ...testDoc.data()} as Test, action: 'result', resultData });
            }
        }
    };
    ...
    const handleConfirmStartTest = () => {
        if (selectedTest) {
            if (user) localStorage.removeItem(`inProgressTest_${user.uid}`);
            onInitiateTestView({ test: selectedTest, action: 'start' });
        }
    ...
    const handleConfirmStartPractice = () => {
        if (selectedTest) {
            if (user) localStorage.removeItem(`inProgressTest_${user.uid}`);
            onInitiateTestView({ test: selectedTest, action: 'practice' });
        }
        setInfoModalOpen(false);
        setSelectedTest(null);
    };

    const getCategoryAndAllChildrenIds = (categoryId: string): string[] => {
        let ids = [categoryId];
    ...
    const filteredTests = React.useMemo(() => {
        return allTests.filter(test => {
            ...
            const categoryMatch = selectedCategory.id ? getCategoryAndAllChildrenIds(selectedCategory.id).includes(test.categoryId) : true;
            const searchMatch = searchQuery ? test.title.toLowerCase().includes(searchQuery.toLowerCase()) : true;
            const sectionMatch = selectedSection === 'All' ? true : test.section === selectedSection;
            return categoryMatch && searchMatch && sectionMatch;
        });
    }, [allTests, selectedCategory, categories, searchQuery, selectedSection]);
                ...
                {testsToRender.length > 0 ? (
                    testsToRender.map(test => (
                        <TestCard
                            ...
                            inProgressTestId={inProgressTest?.testId}
    ...
    const renderHomepageComponent = (component: HomeComponent) => {
        ...
        switch (component.type) {
            ...
            case 'latest_tests': {
                ...
                const tests = allTests.filter(isTestLive).slice(0, config.limit);
                return (
                    <section>
                            ...
                            {topLevelCategories.map(cat => {
                                ...
                                <button key={cat.id} onClick={() => onSelectCategory({ id: cat.id, name: cat.name })} className="block p-4 bg-white dark:bg-gray-800 rounded-xl shadow-md border dark:border-gray-700 hover:border-indigo-400 dark:hover:border-indigo-500 hover:shadow-lg transition-all text-center group">
                                    <div className={`w-24 h-24 rounded-full mx-auto mb-4 border-4 border-white dark:border-gray-800 shadow-md transform group-hover:scale-110 transition-transform duration-300 flex items-center justify-center ${style.bg} ${style.text}`}>
                                        <DynamicIcon name={cat.name} className="w-12 h-12" />
                                    </div>
                                    <p className="font-semibold text-gray-800 dark:text-gray-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{cat.name}</p>
                                </button>
                            )})}
                        </div>
                    </section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {config.testimonials.map((t, i) => (
                                <div key={i} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border dark:border-gray-700">
                                    <Quote className="text-indigo-200 dark:text-indigo-800" size={32}/>
                                    <p className="text-gray-600 dark:text-gray-300 my-4 italic">"{t.text}"</p>
                                    <div className="text-right">
                                        <p className="font-semibold text-gray-800 dark:text-gray-200">{t.author}</p>
    ...
    const currentFilterName = searchQuery ? `Search results for "${searchQuery}"` : `${selectedCategory.name} Tests`;

    const isInitialLoad = homepageSettings === null && !searchQuery && !selectedCategory.id;
        ...
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="flex-1 space-y-12">
                     {userProfile?.role === 'admin' && (
                        <div className="bg-indigo-50 dark:bg-indigo-900/20 border-2 border-dashed border-indigo-200 dark:border-indigo-500/30 p-6 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                                <Shield className="w-10 h-10 text-indigo-500 dark:text-indigo-400" />
                                <div>
                                    <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200">Admin Controls</h3>
                                    <p className="text-gray-600 dark:text-gray-400">Access the dashboard to manage the application.</p>
                                </div>
                            </div>
                    ...
                    {user && !loadingRecommendations && recommendedTests.length > 0 && !searchQuery && !selectedCategory.id && (
                        <section>
                            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6 flex items-center gap-3">
                                <Star className="text-amber-500" /> Recommended For You
                            </h2>
                            <p className="text-gray-600 dark:text-gray-400 -mt-4 mb-6">Based on your performance, you might want to try these tests to improve.</p>
                            {renderTestGrid(recommendedTests, "")}
                        </section>
                    )}

                    {!searchQuery && !selectedCategory.id && (homepageSettings?.layout || []).map(comp => 
            ...
            {selectedTest && (
                <TestInfoModal 
                    ...
                    onStartPractice={handleConfirmStartPractice}
                    test={selectedTest}
                />
            )}
        </div>
