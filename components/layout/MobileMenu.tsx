...
import { X, Home, ChevronRight, FolderKanban } from 'lucide-react';
    ...
    return (
        <li>
            <button
                ...
                className="w-full flex items-center justify-between p-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors duration-200 text-left"
            >
                <span className="flex items-center flex-grow relative">
                    <DynamicIcon name={category.name} className={`w-5 h-5 flex-shrink-0 ${style.text}`} />
            ...
            {hasChildren && isOpen && (
                <ul className="pl-6 ml-3 mt-1 space-y-1 border-l-2 border-gray-200 dark:border-gray-700 animate-fade-in">
                    <li>
                        <button onClick={() => onSelectCategory({ id: category.id, name: category.name })} className="w-full text-left flex items-center p-2 text-gray-600 dark:text-gray-400 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/30">
                    ...
                    {childCategories.map(child => {
                        ...
                        return (
                        <li key={child.id}>
                            <button onClick={() => onSelectCategory({ id: child.id, name: child.name })} className="w-full text-left flex items-center p-2 text-gray-600 dark:text-gray-400 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/30">
                                <DynamicIcon name={child.name} className={`w-4 h-4 mr-2 flex-shrink-0 ${childStyle.text}`} />
...
const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, onClose, categories, loading, onSelectCategory }) => {
    ...
    return (
        <div className={`lg:hidden fixed inset-0 z-50 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
            {/* Overlay */}
            <div 
                className={`fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 transition-opacity duration-300 ease-in-out ${isOpen ? 'opacity-100' : 'opacity-0'}`} 
                onClick={onClose}
                aria-hidden="true"
            ></div>
            ...
            <div className={`fixed top-0 left-0 h-full bg-white dark:bg-gray-800 shadow-2xl z-50 transition-transform duration-300 ease-in-out w-[75%] sm:w-48 border-r border-gray-200 dark:border-gray-700 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <div className="p-4 overflow-y-auto h-[calc(100vh-65px)]">
                    <ul className="space-y-1">
                        <li>
                            <button onClick={() => onSelectCategory({ id: '', name: 'All Tests' })} className="w-full flex items-center p-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors duration-200 relative sidebar-link">
