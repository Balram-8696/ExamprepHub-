...
const ManageReports: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (filter === 'all') {
            ...
            q = query(collection(db, 'reports'), where('status', '==', filter), orderBy('reportedAt', 'desc'));
        }
    ...
    const FilterButton: React.FC<{ status: ReportStatus | 'all', label: string, color: string }> = ({ status, label, color }) => {
        ...
        return (
            <button
                ...
                className={`px-4 py-2 text-sm font-semibold rounded-lg transition-all ${isActive ? `${color} text-white shadow` : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}
            >
                {label}
            </button>
        );
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-lg border-t-4 border-indigo-500">
            <h1 className="text-3xl font-extrabold text-gray-900 dark:text-gray-100 flex items-center gap-3 mb-6">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {reports.length === 0 ? (
                                <tr><td colSpan={6} className="text-center py-10 text-gray-500 dark:text-gray-400">No reports found for this filter.</td></tr>
                            ) : reports.map(report => (
                                <tr key={report.id}>
                                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900 dark:text-gray-200">{report.testTitle} (Q{report.questionIndex + 1})</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{report.reason}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{report.userEmail}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{report.reportedAt ? new Date(report.reportedAt.seconds * 1000).toLocaleDateString() : 'N/A'}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm"><StatusBadge status={report.status} /></td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                                        <button onClick={() => handleViewDetails(report)} className="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 font-semibold">View Details</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                <Modal isOpen={!!selectedReport} onClose={() => setSelectedReport(null)} title="Report Details" size="lg">
                    <div className="space-y-4 text-sm">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div><p className="font-semibold text-gray-700 dark:text-gray-300">User:</p><p className="text-gray-600 dark:text-gray-400">{selectedReport.userName} ({selectedReport.userEmail})</p></div>
                            <div><p className="font-semibold text-gray-700 dark:text-gray-300">Test:</p><p className="text-gray-600 dark:text-gray-400">{selectedReport.testTitle} (Q{selectedReport.questionIndex + 1})</p></div>
                        </div>
                        <div><p className="font-semibold text-gray-700 dark:text-gray-300">Question:</p><p className="p-2 bg-gray-100 dark:bg-gray-700 rounded-md text-gray-800 dark:text-gray-200">{selectedReport.questionText}</p></div>
                        <div><p className="font-semibold text-gray-700 dark:text-gray-300">Reason:</p><p className="text-gray-600 dark:text-gray-400">{selectedReport.reason}</p></div>
                        {selectedReport.comments && <div><p className="font-semibold text-gray-700 dark:text-gray-300">User Comments:</p><p className="p-2 bg-gray-100 dark:bg-gray-700 rounded-md text-gray-800 dark:text-gray-200">{selectedReport.comments}</p></div>}
                        
                        <div className="border-t pt-4 dark:border-gray-600">
                            <label className="block text-sm font-medium mb-1 dark:text-gray-300">Admin Reply (optional)</label>
                            <textarea
                                value={adminReply}
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
                            <div className="flex gap-2">
                                <p className="font-semibold text-gray-700 dark:text-gray-300 self-center">Set status to:</p>
