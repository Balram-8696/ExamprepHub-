        ...
        <div className="space-y-8">
                    ...
                    <ul className="space-y-3">
                        {categories.filter(c => !c.parentId).map(parent => {
                            ...
                            <li key={parent.id} className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border dark:border-gray-600">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-md flex items-center justify-center ${parentStyle.bg} ${parentStyle.text}`}>
                                            <DynamicIcon name={parent.name} className="w-5 h-5"/>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => handleOpenModal(parent)} className="text-blue-500 hover:text-blue-700 p-1.5 rounded-full hover:bg-blue-100 dark:hover:bg-gray-600 disabled:opacity-50" disabled={isSubmitting}><Pencil size={16} /></button>
                                        <button onClick={() => handleDeleteClick(parent)} className="text-red-500 hover:text-red-700 p-1.5 rounded-full hover:bg-red-100 dark:hover:bg-gray-600 disabled:opacity-50" disabled={isSubmitting}><Trash2 size={16} /></button>
                                    </div>
                                </div>
                                
                                <div className="pl-6 ml-3 mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                                    <h4 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-2">Sections</h4>
                                    <div className="flex flex-wrap gap-2 mb-3">
                                        {(parent.sections || []).map(sec => (
                                            <div key={sec} className="flex items-center bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium pl-2.5 pr-1 py-1 rounded-full">
                                                <span>{sec}</span>
                                                <button onClick={() => handleRemoveSection(parent.id, sec)} className="ml-1.5 -mr-1 p-0.5 rounded-full hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">
                                                    <X size={14} />
                                                </button>
                                            </div>
                                        ))}
                                    ...
                                    <form onSubmit={(e) => handleAddSection(e, parent.id)} className="flex gap-2 items-center">
                                        <input 
                                            ...
                                            className="flex-grow p-1.5 border border-gray-300 dark:border-gray-500 rounded-lg text-sm bg-white dark:bg-gray-700"
                                        />
                                ...
                                <ul className="pl-6 mt-2 space-y-2 font-normal text-gray-600 dark:text-gray-300 border-l-2 border-indigo-200 dark:border-indigo-500/50 ml-3">
                                    {categories.filter(c => c.parentId === parent.id).length > 0 ? categories.filter(c => c.parentId === parent.id).map(child => {
                                        ...
                                        <li key={child.id} className="pt-1">
                                            <div className="flex justify-between items-center p-2 rounded-lg hover:bg-white dark:hover:bg-gray-600/50">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-5 h-5 rounded-md flex items-center justify-center shrink-0 ${childStyle.bg} ${childStyle.text}`}>
                                                        <DynamicIcon name={child.name} className="w-4 h-4"/>
                                                    </div>
                                                    <span>{child.name}</span>
                                                    <span className="text-xs font-mono bg-gray-200 dark:bg-gray-600 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full">{testCounts[child.id] || 0} tests</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                     <button onClick={() => handleOpenModal(child)} className="text-blue-500 hover:text-blue-700 p-1.5 rounded-full hover:bg-blue-100 dark:hover:bg-gray-500 disabled:opacity-50" disabled={isSubmitting}><Pencil size={16} /></button>
                                                     <button onClick={() => handleDeleteClick(child)} className="text-red-500 hover:text-red-700 p-1.5 rounded-full hover:bg-red-100 dark:hover:bg-gray-500 disabled:opacity-50" disabled={isSubmitting}><Trash2 size={16} /></button>
                                                </div>
                                            </div>
                                        </li>
            ...
            >
                <form onSubmit={handleSaveCategory} className="space-y-4">
                    <div>
                        <label htmlFor="category-name" className="block text-sm font-medium mb-1 dark:text-gray-300">Category Name</label>
                        <input
                            id="category-name"
                            type="text"
                            value={formName}
                    ...
                    <div>
                        <label htmlFor="parent-category" className="block text-sm font-medium mb-1 dark:text-gray-300">Parent Category</label>
                        <select
                            id="parent-category"
                            value={formParentId}
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        <button type="button" onClick={handleCloseModal} className="px-4 py-2 bg-gray-200 text-gray-800 font-semibold rounded-lg hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500">Cancel</button>
