...
import { LayoutDashboard, FolderKanban, FilePlus, Palette, ListChecks, BookText, X, Flag } from 'lucide-react';
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ currentView, onSetView, isOpen, onClose }) => {
    ...
    return (
        <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col overflow-y-auto border-r border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-700 transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="flex shrink-0 items-center justify-between border-b dark:border-gray-700 px-4 h-16">
                ...
                <button onClick={onClose} className="lg:hidden p-1 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white">
                    <X size={20} />
                </button>
            </div>
